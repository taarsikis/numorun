...
struct BalanceView: View {
    ...
    @StateObject private var paychecksViewModel = PaychecksViewModel()
...
//                      navigateToPayment = true
                ...
                .cornerRadius(12)
                  ...
                  .fullScreenCover(isPresented: $navigateToPayment, content: {
                      PaymentView()
                  })
                Spacer()
                  Button(action:{
              ...
              .padding(.horizontal,15)
                ...
                // --------------------------------------
                    if paychecksViewModel.paychecks.filter{$0.userID == self.userID}.count == 0{
                        Text("Ви ще не проводили транзакції")
                          .foregroundColor(Color(hex: "#1B1C1C"))
                          .font(.system(size: 18))
                          .padding(.bottom,5)
                    }
                    if loading{
                        ...
                        ForEach(paychecksViewModel.paychecks.filter{$0.userID == self.userID}){ paycheck in
                                ...
                                HStack(spacing: 0){
                                  ...
                                  VStack(alignment: .leading, spacing: 0){
                                    Text("Поповнення балансу")
                                      ...
                                      .font(.system(size: 18))
                                      .padding(.bottom,5)
                                      Text((paycheck.createdDate.toDate(withFormat: "yyyy-MM-dd'T'HH:mm:ss.SSSSSS") ?? Date() ).toString(withFormat: "yyyy MMMM dd, HH:mm"))
                                        .foregroundColor(Color(hex: "#5D5E5E"))
                                        .font(.system(size: 12))
                                        .padding(.bottom,7)
                                  }
                            ...
                            if paycheck.status == "success-out"{
                                HStack(spacing: 0){
                              VStack(alignment: .leading, spacing: 0){
            ...
            .onAppear {
                ...
                paychecksViewModel.fetchPayChecks(){_ in 
                    loading = false
                }
            }
    }
